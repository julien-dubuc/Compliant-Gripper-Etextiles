...
%% 1. SETTINGS
file_basename = 'CM_D_'; % C1_D_, C2_D_, C3_D_ , CM_D_
...
max_deviation = 50; % mm - intra-trial outlier filter
match_thresh  = 20; % mm - max distance for marker matching
header_lines  = 7;
...
slope_win_s  = 1.0; 
slope_thresh = 0.15; 
plateau_min_dur_s = 1.0; 
plateau_merge_gap_s = 0.5; 
merge_value_tol = 0.15; 
merge_gap_max_s = 2.0; 
post_release_tol_factor = 3; 
...
for trial = 1:num_trials
    ...
    if ~isfile(filename)
        warning('File not found: %s. Skipping trial.', filename);
        continue;
    end
...
name_map = containers.Map(fr_names, marker_names);
...
for trial = 1:num_trials
    if isempty(aperture{trial}) || all(isnan(aperture{trial})), continue; end
    ...
    if n_plat_total < 2, continue; end
    ...
    stage_range{trial} = plats(sel, :);
    stage_label{trial} = labels;
    stage_value{trial} = vals_all(sel);
end
...
%% 6. HYSTERESIS & DEFORMATION SUMMARY
% NOTE: cumulative drift is referenced to Grip MAX, not Grip min.
% Grip min depends on the operator manually placing the cylinder between
% the fingers before each trial, so it is not a repeatable reference -
% any variation in manual placement shows up as "drift" that has nothing
% to do with the gripper itself. Grip max is reached by the motor
% driving to a fixed target every time, so it is a much more repeatable
% reference for measuring genuine mechanical drift across trials.
...
ref_close    = nan(num_markers, 2); % Grip max position of trial 1 (reference)

for trial = 1:num_trials
    if isempty(stage_pos{trial}), continue; end
    if trial == 1
        ref_close = squeeze(stage_pos{trial}(:,:,2));
    end
end
...
for trial = 1:num_trials
    ...
    for m = 1:num_markers
        ...
        if ~isnan(ref_close(m,1))
            dxc = pos(m,1,2) - ref_close(m,1);
            dyc = pos(m,2,2) - ref_close(m,2);
...
title(tl2, sprintf('Gripper %s : Hysteresis & Deformation Summary', gripper_label), 'FontSize', 14, 'FontWeight', 'bold');
...
%% 8. PLOT: Y(t) EVOLUTION PER MARKER
gap_s = 1.0;
...
title(t_layout, sprintf('Gripper %s : Y evolution from Grip min to Grip max', gripper_label), ...
...
for step = 1:num_markers
    ...
    if ~isempty(y_all_valid)
        ...
        yticklabels(string(round(yticks, 1))); 
    end

clc; clear; close all;
file_basename   = 'CM_D_'; % (C1_D_, C2_D_, C3_D_)
...
% FIX: BR et MRR etaient quasiment indiscernables (deux jaunes/ambres tres
% proches). BR passe en anthracite fonce pour un contraste net avec tous
...
colors = [
    ...
    0.64, 0.08, 0.18; % MLL
    0.00, 0.50, 0.50; % MR
    0.95, 0.85, 0.00  % BR (etait [0.95 0.85 0.00], confondu avec MRR)
];
...
post_release_tol_factor = 3; % un palier apres Grip max est considere "retour a Grip min"
                              % si son aperture est a moins de
                              % post_release_tol_factor*merge_value_tol
                              % de l'aperture de Grip min (pre-fermeture)
...
% DETECTION DES PALIERS : "Grip min", "Grip max", et "Grip min (post)" si present
stage_range = cell(1, num_trials);
stage_label = cell(1, num_trials);
stage_value = cell(1, num_trials);

fprintf('\n=== Detection des paliers par essai (Grip min / Grip max [/ post]) ===\n');
for trial = 1:num_trials
    ...
    % NOUVEAU : si un palier existe APRES Grip max et que son ouverture
    % ressemble a celle de Grip min (pre-fermeture), on le garde comme
    % "Grip min (post)" -> permet de mesurer le retour a l'ouverture
    % apres relachement, dans le MEME essai (deficit de recuperation).
    has_post = false;
    if idx_max < n_plat_total
        if abs(vals_all(idx_max+1) - vals_all(idx_max-1)) < post_release_tol_factor*merge_value_tol
            has_post = true;
        end
    end

    if has_post
        sel    = idx_max-1 : idx_max+1;
        labels = {'Grip min', 'Grip max', 'Grip min (post)'};
    else
        sel    = idx_max-1 : idx_max;
        labels = {'Grip min', 'Grip max'};
    end
    plats = plats(sel, :);
    vals  = vals_all(sel);

    fprintf('--- Trial %d (%d paliers au total detectes, %d retenus) ---\n', trial, n_plat_total, numel(sel));
    for k = 1:numel(labels)
        fprintf('  %-15s : t=[%.2f, %.2f]s  aperture=%.2f mm  (n=%d frames)\n', ...
...
%% ========================================================================
%  6b. HYSTERESIS & DEFORMATION SUMMARY (NOUVEAU)
%  ========================================================================
% Trois grandeurs par marqueur, en fonction du numero d'essai (1..num_trials).
% On utilise la MAGNITUDE du deplacement (hypot(dX,dY)) plutot que dY seul :
% certains marqueurs voient leur Y augmenter en fermant, d'autres diminuer
% (ca depend juste de leur position sur le gripper) - melanger des
% grandeurs signees de sens opposes sur un seul graphe induit en erreur.
%
%   (A) Closing amplitude = |Grip max - Grip min|            (par essai)
%   (B) Recovery deficit  = |Grip min (post) - Grip min|     (par essai,
%       seulement si un palier de retour a ete detecte pour cet essai)
%   (C) Cumulative drift  = |Grip min(essai n) - Grip min(essai 1)|
closing_amp   = nan(num_markers, num_trials);
recovery_def  = nan(num_markers, num_trials);
cum_drift     = nan(num_markers, num_trials);
has_recovery  = false(1, num_trials);

ref_open = nan(num_markers, 2); % position Grip min de l'essai 1 (reference)
for trial = 1:num_trials
    if isempty(stage_pos{trial}), continue; end
    if trial == 1
        ref_open = squeeze(stage_pos{trial}(:,:,1));
    end
end
...
for trial = 1:num_trials
    ...
    for m = 1:num_markers
        dx = pos(m,1,2) - pos(m,1,1);
        dy = pos(m,2,2) - pos(m,2,1);
        closing_amp(m,trial) = hypot(dx, dy);

        if n_plat >= 3
            dxr = pos(m,1,3) - pos(m,1,1);
            dyr = pos(m,2,3) - pos(m,2,1);
            recovery_def(m,trial) = hypot(dxr, dyr);
            has_recovery(trial) = true;
        end

        if ~isnan(ref_open(m,1))
            dxc = pos(m,1,1) - ref_open(m,1);
            dyc = pos(m,2,1) - ref_open(m,2);
            cum_drift(m,trial) = hypot(dxc, dyc);
        end
    end
end

n_panels = 2 + any(has_recovery);
figure('Name', ['Gripper ' gripper_label ' - Hysteresis Summary'], 'Color', 'w', 'Position', [100 100 1500 500]);
tl2 = tiledlayout(1, n_panels, 'TileSpacing', 'compact', 'Padding', 'compact');
title(tl2, sprintf('Gripper %s : Hysteresis & Deformation Summary (Test B, %d trials)', gripper_label, num_trials), ...
    'FontSize', 14, 'FontWeight', 'bold');

panel = 1;
nexttile(panel); hold on; grid on;
for m = 1:num_markers
    plot(1:num_trials, closing_amp(m,:), '-o', 'Color', colors(m,:), 'LineWidth', 1.5, ...
        'MarkerFaceColor', colors(m,:), 'MarkerSize', 5, 'DisplayName', marker_names{m});
end
title('Closing amplitude |Grip max - Grip min|', 'FontSize', 11);
xlabel('Trial number'); ylabel('mm'); xlim([0.5, num_trials+0.5]); xticks(1:num_trials);
panel = panel + 1;

if any(has_recovery)
    nexttile(panel); hold on; grid on;
    for m = 1:num_markers
        plot(1:num_trials, recovery_def(m,:), '-o', 'Color', colors(m,:), 'LineWidth', 1.5, ...
            'MarkerFaceColor', colors(m,:), 'MarkerSize', 5, 'DisplayName', marker_names{m});
    end
    yline(0, '--', 'Color', [0.3 0.3 0.3]);
    title('Recovery deficit |Grip min_{post} - Grip min|', 'FontSize', 11, 'Interpreter', 'tex');
    xlabel('Trial number'); ylabel('mm'); xlim([0.5, num_trials+0.5]); xticks(1:num_trials);
    panel = panel + 1;
else
    fprintf(['\nNB : aucun palier de retour a "Grip min" detecte apres "Grip max" dans ces essais ' ...
        '-> pas de panneau "Recovery deficit" (le protocole de cet essai ne capture peut-etre pas le relachement).\n']);
end

nexttile(panel); hold on; grid on;
for m = 1:num_markers
    plot(1:num_trials, cum_drift(m,:), '-o', 'Color', colors(m,:), 'LineWidth', 1.5, ...
        'MarkerFaceColor', colors(m,:), 'MarkerSize', 5, 'DisplayName', marker_names{m});
end
title('Cumulative drift |Grip min - Trial 1|', 'FontSize', 11);
xlabel('Trial number'); ylabel('mm'); xlim([0.5, num_trials+0.5]); xticks(1:num_trials);
legend(marker_names, 'Location', 'eastoutside', 'FontSize', 8);

exportgraphics(gcf, [gripper_label '_D_Hysteresis.png'], 'Resolution', 600);

% FIGURE : POSITIONS Grip min / Grip max (3 essais)
...
for trial = 1:num_trials
    ...
    for m = 1:num_markers
        ...
        for k = 1:numel(Xi)
            ...
            sm = stage_markers{min(k, numel(stage_markers))};
            scatter(Xi(k), Yi(k), 45, sm, 'MarkerEdgeColor', 'k', ...
...
h_s3 = scatter(NaN,NaN,45,'s','MarkerEdgeColor','k','MarkerFaceColor',[.5 .5 .5]);
legend([h_leg_marker(ok), h_s1, h_s2, h_s3], [marker_names(ok), {'Grip min','Grip max','Grip min (post)'}], ...
    'Location', 'eastoutside', 'FontSize', 9);

% FIGURE : Y(t) PAR MARQUEUR EN SOUS-GRAPHES
gap_s = 1.0;
...
for step = 1:num_markers
    ...
    if ~isempty(y_all_valid)
        ...
        yticklabels(string(round(yticks, 1))); % Arrondi a 1 decimale
    end
...
exportgraphics(figure(3), [gripper_label '_Deformation.png'], 'Resolution', 600);
